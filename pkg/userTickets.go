...
func GetUserTickets(assignee string) ([]string, []string, []string) {

	url := "https://forgerock.zendesk.com/api/v2/search.json?query=" + url.QueryEscape("type:ticket assignee:\""+assignee+"\" status:open status:pending")
	...
	userTickets := make([]string, len(myTicket.Tickets))

	for i := range userTickets {
		userTickets[i] = strconv.Itoa(myTicket.Tickets[i].ID)
	}
	...
	//jsonErr := json.Unmarshal(body, &myTicket)
	return userTickets, ticketPriority, ticketStatus
